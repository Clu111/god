...
class Program
{
    ...
    private static string token = "8137272571:AAF7a4muYUOrOXPW3UifwcKOIbMAkz9j0NI";
    ...
    private static async Task UpdateHandler(ITelegramBotClient client, Update update, CancellationToken token)
    {
        if (update.Type == UpdateType.Message && update.Message?.Text != null)
        {
            ...
            List<long> adminIds = new List<long> { 1910620149 };
    ...
    private static async Task HandleAdminCommands(ITelegramBotClient client, Message message)
    {
        ...
        if (message.Text.StartsWith("/create"))
        {
            ...
            if (parts.Length < 2)
            {
                await client.SendTextMessageAsync(chatId, "Использование: /create НазваниеРозыгрыша");
                return;
            }
